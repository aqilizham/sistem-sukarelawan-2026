...
using (
  ...
  or (public.current_role() = 'Ketua Unit' and unit is not distinct from public.current_unit())
);

drop policy if exists profiles_insert_self on public.profiles;
create policy profiles_insert_self on public.profiles
for insert to authenticated
with check (id = auth.uid() or public.is_admin_induk());
...
using (id = auth.uid() or public.is_admin_induk())
with check (id = auth.uid() or public.is_admin_induk());
...
using (
  public.is_admin_induk()
  or (target_role is null and target_cluster is null and target_venue is null)
